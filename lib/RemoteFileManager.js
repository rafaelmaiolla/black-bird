...
class RemoteFileManager extends EventEmitter {
  ...
  createFile(uri) {
    ...
    if (!remoteFile.getLocalPath()) {
      remoteFile.createLocalPath();
    }
  ...
  static fixUri(uri) {
    return RemoteFileManager.PROTOCOL + "//" + uri.replace(RemoteFileManager.PROTOCOL + "\\", "");
  }
