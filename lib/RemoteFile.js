...
class RemoteFile extends EventEmitter {
  ...
  readSync() {
    return fs.readFileSync(this.getLocalPath());
  }

  createLocalPath() {
    this.localPath = path.join(os.tmpdir(), uuid.v4(), path.basename(this.filePath));
  }
  ...
  edit() {
    ...
    atom.workspace.open(this.getLocalPath()).then((editor) => {
