...
class Session extends EventEmitter {
  ...
  commandList(fileList, remotePath) {
    console.log('[black-bird]', 'File list', fileList);

    this.emit('listed');

    // TODO: Create tree in project
    var options = {basename: "List: " + path.basename(remotePath)};
    var fd = this.createTemporaryFile(options);
  ...
  commandDiff(diff) {
    ...
    var options = {basename: ".diff"};
    var fd = this.createTemporaryFile(options);
    fs.writeSync(fd, diff);
    fs.closeSync(fd);
    this.atomOpenFile(options);
  }
  ...
  commandCreated(basename) {
    console.log('[black-bird]', 'File created', basename);
    this.emit('created', basename);
  }

  commandRemoved(basename) {
    console.log('[black-bird]', 'File removed', basename);
  ...
  listFiles(remotePath, ignoredNames) {
    console.log('[black-bird]','List files', remotePath, ignoredNames);
    this.emit('listing');
    this.sendCommand('List', [remotePath, ignoredNames]);
  }

  diffFiles(remotePath, respositoryType) {
    console.log('[black-bird]','Diff files', remotePath, respositoryType);
    this.emit('diffing');
    this.sendCommand('Diff', [remotePath, respositoryType]);
  }
